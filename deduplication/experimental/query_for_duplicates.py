...
def parse_args():
    ...
    parser.add_argument(
        "-t",
        "--tag",
        help="Tag, used when logging profiling information (e.g. rp1_vs_arxiv)",
        required=True,
    )
    parser.add_argument(
        "--nodeid",
        help=f"Idx of the current node",
        type=str,
        required=False,
        default="00",
    )
    parser.add_argument(
        "--redis_port",
        help=f"Port for local Redis server. Default is 6379.",
        type=int,
        required=False,
        default=6379,
    )
    parser.add_argument(
        "--profile-fp",
        help=f"Whether to profile false positives. This will slow down execution and should only be used for profiling.",
        action="store_true",
    )
